    ...
    public class JobCardService : IJobCardService
    {
            ...
            try
            {
                ...
                if (request.MaterialRequirements != null && request.MaterialRequirements.Any())
                {
                    var materialRequirements = request.MaterialRequirements.Select(mr => new JobCardMaterialRequirement
                    {
                        ...
                        Unit = mr.Unit.Trim(),
                        WastageMM = mr.WastageMM,
                        // Add wastage in mm to required quantity (assumes both are in same units)
                        TotalQuantityWithWastage = mr.RequiredQuantity + mr.WastageMM,
        ...
        private static JobCardResponse MapToResponse(JobCard jobCard)
        {
            return new JobCardResponse
            {
                ...
                ProductionStatus = jobCard.ProductionStatus,
